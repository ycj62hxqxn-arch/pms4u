    ...
    <main className={styles.main}>
      ...
      <div style={{marginBottom: 24, padding: 16, background: "#f5f5f5", borderRadius: 8, border: "1px solid #e0e0e0"}}>
        {loading ? (
          ...
          <>
            <div><strong>Correlation ID:</strong> <span style={{color: "#1976d2"}}>{latestEvidence.correlation_id}</span></div>
            <div><strong>Evidence ID:</strong> {latestEvidence.evidence_id}</div>
            <div><strong>Timestamp:</strong> {latestEvidence.timestamp}</div>
            <div><strong>Authority Chain:</strong> {latestEvidence.authority_chain?.join(" → ")}</div>
            <div><strong>Verdict:</strong> <span style={{color: latestEvidence.verdict === "ALLOW" ? "#388e3c" : latestEvidence.verdict === "DENY" ? "#b71c1c" : latestEvidence.verdict === "HOLD" ? "#fbc02d" : "#1976d2"}}>{latestEvidence.verdict}</span></div>
            <div><strong>Reason:</strong> {latestEvidence.reason}</div>
            <div><strong>Mutation Status:</strong> {latestEvidence.mutation_status}</div>
            <div><strong>Consequence Domain:</strong> {latestEvidence.consequence_domain}</div>
            <div><strong>Runtime Hash:</strong> {latestEvidence.runtime_hash}</div>
            <div><strong>Execution Trace:</strong> {Array.isArray(latestEvidence.execution_trace) ? latestEvidence.execution_trace.join(" → ") : latestEvidence.execution_trace}</div>
            <div><strong>Ledger Position:</strong> {latestEvidence.ledger_position}</div>
            <div><strong>Seal State:</strong> {latestEvidence.seal_state}</div>
            <div><strong>Prev Hash:</strong> <span style={{fontFamily: "monospace", fontSize: 12}}>{latestEvidence.prev_hash}</span></div>
            <div><strong>Current Hash:</strong> <span style={{fontFamily: "monospace", fontSize: 12}}>{latestEvidence.current_hash}</span></div>
          </>
        ) : null}
      </div>
